...
# from TimeSeriesForecastTask import TimeSeriesForecastTask
from LinearTimeSeriesForecastTask import TimeSeriesForecastTask
...
#     current_time = time.time()
#     logging.format_logger(L.pytorch._logger)
#     cli = CustomLightningCLI(TimeSeriesForecastTask, YahooFinanceDataModule, run=False)  # noqa: F841
#     cli.trainer.fit(cli.model, datamodule=cli.datamodule)
#     cli.trainer.test(ckpt_path="best", datamodule=cli.datamodule)
#     print("Done training: Time elapsed: {:.2f}s".format(time.time() - current_time))
